...
#       }
    ...
    if(length(select_all_text_vars) > 0) {
      ...
      purrr::walk(select_all_text_vars, ~{
        df_to_use[[paste0("text_", .x)]] <- df_to_use[[.x]]
      })
    }
    ...
    if(numeric_included == TRUE) {
      ...
      purrr::walk(unique_scale_names_to_use, ~{
        label_for_the_score <- paste0("AVERAGE_SCORE__", snakecase::to_snake_case(.x))
        final_scale_variable_names <- scales_details %>%
          dplyr::filter(.data[["final_scale_names"]] == .x) %>%
  ...
  if(any(check_for_select_all_vars)){
    ...
    # Obtain the calculations for every df in the list
    wave_tables_df <- purrr::map2_df(all_dfs, as.list(names(all_dfs)), ~{
      perform_calc_create_table_single_sa(df = .x, WaveName = .y, parameter_df, column_workbook = column_workbook_lists_single)
